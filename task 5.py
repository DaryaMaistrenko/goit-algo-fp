...
def draw_tree(tree_root):
    ...
    nx.draw(tree, pos, labels=labels, node_color=colors, with_labels=True, node_size=2500)
...
def dfs(root):
    ...
    while stack:
        current = stack.pop()
        order.append(current)
        if current.right:  # Спочатку відвідуємо праве піддерево
            stack.append(current.right)
        if current.left:  # Потім - ліве
...
def assign_colors(nodes):
    ...
    for i, node in enumerate(nodes):
        shade = int(255 * (i / (num_nodes - 1)))  # Генерація відтінків
